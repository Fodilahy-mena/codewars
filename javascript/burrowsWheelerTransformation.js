...
function encode(s) {
    ...
    let matrix = [s]
    for(let i = s.length - 1; i; i--) matrix.push(s.slice(i).concat(s.slice(0, i)))
    matrix = matrix.sort()
    
    const index = matrix.indexOf(s)
    const lastColumn = matrix.map(row => row[row.length - 1]).join``
    return [lastColumn, index]
}
  
function decode(s,i) {
    if (!s) return '';
    const firstColumn = s.split``.sort()
    let matrix = firstColumn.map((sym, index) => s[index] + sym)

    while(matrix[0].length !== matrix.length) {
        matrix = matrix.sort()
        matrix = matrix.map((substr, index) => s[index] + substr)
    }
    matrix = matrix.sort()
